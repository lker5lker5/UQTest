...
Host On Web Server:
    => http://elipseapplicationassignment-kaner.rhcloud.com/index.php
...
———>2.2. Setup on host server
    ...
    *However, there are some changes you need to make, the custom.js and admin.js
     |
     |-> those AJAX code parts, we need to change the location in both files
	 window.location.href.replace(/admin.*/g,'') + “dataHandler/….”
	  |-> to window.location.href + “/dataHandler/….”
     |-> This is because in server, the URL covers the IP.Add/index.php
